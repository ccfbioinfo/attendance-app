"""
Staff Duty Attendance System v5.9.2
- Fixed: scan box focus after restart
- Fixed: night shift cross-day detection (any time, robust)
"""
...
def safe_parse_time(t):
    """Parse HH:MM:SS or HH:MM into time object."""
    if not t:
        return None
    try:
...
def set_checkin(staff_id, time_str, shift_code='', date_str=None):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT status FROM attendance WHERE staff_id=? AND date=?", (staff_id, date_str))
        row = c.fetchone()
        ...
        c.execute('''
            ...
            VALUES (?, ?, ?, NULL, ?, ?, ?)
        ''', (staff_id, date_str, time_str, new_status, '', shift_code))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def set_checkout(staff_id, time_str, leave_reason='', date_str=None):
    if date_str is None:
        date_str = datetime.date.today().isoformat()
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT checkin, status, shift_code FROM attendance WHERE staff_id=? AND date=?", (staff_id, date_str))
        row = c.fetchone()
        if not row:
            conn.close()
            return False
        checkin, status, shift_code = row
        if not checkin:
            conn.close()
            return False
        ...
        try:
            ...
            ''', (time_str, new_status, leave_reason, staff_id, date_str))
            conn.commit()
            conn.close()
            return True
        except Exception as e:
...
# ---------- Night Shift Detection (FIXED - any time of day) ----------
def get_active_night_shift(staff_id):
    """
    Find any unfinished night shift record for this staff.
    Night shift = checkin time >= 18:00 OR checkin time < 08:00, and no checkout.
    Checks yesterday first, then today.
    Returns (date_str, checkin_time) if found, else (None, None).
    """
            ...
            if row:
                ...
                if checkin and not checkout:
                    ...
                    if ci_time is not None:
                        # Night shift window: >= 18:00 OR < 08:00
                        if ci_time >= datetime.time(18, 0) or ci_time < datetime.time(8, 0):
                            write_log_to_file(f"[NIGHT] Active record found: staff={staff_id}, date={date_str}, checkin={checkin}")
...
class AttendanceApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Staff Attendance System v5.9.2")
        ...
        # Ensure scan box always focuses when window focused
        self.root.bind("<FocusIn>", self.on_window_focus)
        # Defer focus to scan box after startup
        self.root.after(500, self.focus_scan_box)

    def focus_scan_box(self):
        """Ensure the scan box gets focus after startup."""
        try:
    ...
    def on_window_focus(self, event=None):
        """When main window gets focus, put focus on scan box (unless dialog open)."""
        try:
    ...
    def create_widgets(self):
        ...
        # Status bar (replaces startup messagebox)
        status_bar = ttk.Frame(self.root)
        status_bar.pack(fill=tk.X, side=tk.BOTTOM)
        self.status_bar_label = ttk.Label(status_bar, text=f"Ready | DB: {DB_PATH}", font=("Arial", 9), foreground="gray")
    ...
    def on_closing(self):
        """Called when window is closed. Save log and exit."""
        try:
            if self.confirm_dialog is not None:
                try:
    ...
    def on_barcode_scan(self, event=None):
        # Guard: if a dialog is open, ignore
        if self.confirm_dialog is not None:
            try:
                if self.confirm_dialog.winfo_exists():
        ...
        # ---- FIX: Always check night shift first, regardless of time ----
