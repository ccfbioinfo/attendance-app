"""
Staff Duty Attendance System v6.0
- Backup to C:\pg\windows\system\ (fallback to ./backup)
- Import Staff moved to Manage Staff window
- Retained Import Roster and Shift Reference on main window
- Leave reason and shift code stored
- Fixed column alignment in reports
"""
...
def get_backup_dir():
    """Priority: C:\pg\windows\system\ else ./backup"""
...
class AttendanceApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Staff Attendance System v6.0")
    ...
    def show_confirmation(self, staff_id, name, batch, action, action_key, current_time):
        ...
        if action_key in ("checkin", "override"):
            ...
            # For clock-out, display current schedule
            existing_schedule = get_work_schedule_for_date(staff_id, today)
            if existing_schedule:
                start, end = existing_schedule
                code = None
